...
from models import db, User, Character,Favourite_list,Species,Planet
...
def setup_admin(app):
    ...
    admin.add_view(AdminView(Character, db.session))
    admin.add_view(AdminView(Species, db.session))
    admin.add_view(ModelView(Planet, db.session))
    admin.add_view(AdminView(Favourite_list, db.session))
